...
import type { MonthlyAttendanceGrid, DayCellStatus } from '../types';
...
export const AttendanceMonthlyGrid: React.FC<AttendanceMonthlyGridProps> = ({
  grid,
  selectedTeacherId,
  onSelectTeacher,
  selectedDate = null,
  ...
  const handleHeaderClick = (isoDate: string) => {
    ...
    // click again on same date to unselect/clear the column highlight
    ...
    <div className="flex flex-col">
              ...
              {days.map((day) => {
                ...
                return (
                  <th
                    ...
                    className={`py-2.5 px-2 text-center min-w-[48px] border-r border-[#E6E8EE] dark:border-[#1E293B] last:border-r-0 normal-case transition-colors ${
                  ...
                  >
                    <div
                      className={`font-bold text-[11px] leading-tight ${
                        isSelected ? 'text-[#2451D9] dark:text-[#60A5FA]' : 'text-[#12141C] dark:text-slate-200'
                      }`}
                    >
                      {day.date}
                    </div>
                    ...
                    <div
                      className={`text-[9px] font-normal leading-tight mt-0.5 ${
                        isSelected ? 'text-[#2451D9]/80 dark:text-[#60A5FA]/80' : 'text-[#9DA2AF] dark:text-slate-500'
                      }`}
                    >
                      {day.weekday}
                    </div>
                  ...
                  >
                    ...
                    {teacher.statuses.map((status, idx) => {
                      ...
                      return (
                        ...
                        >
                          {status === 'Present' && (
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#E7F7ED] dark:bg-emerald-500/15 text-[#16A34A] dark:text-emerald-400 text-[9px] font-bold whitespace-nowrap">
                          ...
                          {status === 'Absent' && (
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#FCEAE4] dark:bg-rose-500/15 text-[#DC5B3E] dark:text-rose-400 text-[9px] font-bold whitespace-nowrap">
                          ...
                          {status === 'NoClass' && (
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-gray-100 dark:bg-slate-700/60 text-[#9DA2AF] dark:text-slate-400 text-[9px] font-bold whitespace-nowrap">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 px-4 py-3 border-t border-[#E6E8EE] dark:border-[#334155] text-[11px] text-[#686E7D] dark:text-[#94A3B8]">
        ...
        <span className="text-[#9DA2AF] dark:text-slate-500">
          Click any date cell to edit &middot; Scroll horizontally to see more days &rarr;
        </span>
      </div>
    </div>
  );
